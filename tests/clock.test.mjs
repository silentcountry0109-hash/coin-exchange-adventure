...
test('genLv2 ×500：以報讀為主(3讀2撥)、分針 5 分格、第1題整刻度暖身、其餘非整刻度、不重複', () => {
  for (let s = 0; s < 500; s++) {
    const ps = CL.genLv2(new CL.Rng(s));
    assert.strictEqual(ps.length, 5);
    assert.deepStrictEqual(ps.map((p) => p.type), ['set', 'read', 'read', 'set', 'read']);
    assert([15, 30, 45].includes(ps[0].m), `第 1 題整刻度暖身: ${ps[0].m}`);
    const used = new Set();
    ps.forEach((p, i) => {
      assert(p.h >= 1 && p.h <= 12);
      assert(p.m % 5 === 0 && p.m >= 5 && p.m <= 55, `m 5 分格且非 0: ${p.m}`);
      // 第 2~5 題用「非整刻度」的難分鐘（不是 0/15/30/45），逼出 4:35 這種
      if (i > 0) assert(![0, 15, 30, 45].includes(p.m), `第 ${i + 1} 題非整刻度: ${p.m}`);
...
test('readOptions ×500：3 個、含正解恰一次、不重複、合法、讀反型必在（合法時）', () => {
  for (let s = 0; s < 500; s++) {
    ...
    // m>=35 時「下一點誤讀」(h+1, m) 會擠掉讀反，故讀反必在僅保證於 m<35
    const nextH = ((h % 12) + 1);
    if (sw && m < 35) {
      assert(opts.some((o) => o.h === sw.h && o.m === sw.m), `讀反型必在: ${h}:${m}`);
    }
    // 干擾項只能是「讀反」「下一點誤讀」或「±5 分」型
    for (const o of opts) {
      if (o.h === h && o.m === m) continue;
      const isSw = sw && o.h === sw.h && o.m === sw.m;
      const isNext = o.h === nextH && o.m === m;
      const p5 = CL.addTime(h, m, 5), m5 = CL.addTime(h, m, -5);
      const isNear = (o.h === p5.h && o.m === p5.m) || (o.h === m5.h && o.m === m5.m);
      assert(isSw || isNext || isNear, `干擾項型別: ${h}:${m} → ${o.h}:${o.m}`);
    }
  }
});

test('readOptions：m>=35 必含「下一點誤讀」干擾 (h+1, m)', () => {
  for (let s = 0; s < 300; s++) {
    const rng = new CL.Rng(s + 77);
    const h = rng.int(1, 12), m = rng.pick([35, 40, 45, 50, 55]);
    const opts = CL.readOptions(h, m, rng);
    const nextH = ((h % 12) + 1);
    assert(opts.some((o) => o.h === nextH && o.m === m), `含下一點誤讀: ${h}:${m} → ${nextH}:${m}`);
  }
});
